Alpenglow Resources (Cited)

- Alpenglow: A New Consensus for Solana — Anza blog: `https://www.anza.xyz/blog/alpenglow-a-new-consensus-for-solana`
- Helius: Alpenglow — Solana's Great Consensus Rewrite: `https://www.helius.dev/blog/alpenglow`
- Alpenglow Presentation Slides — Roger Wattenhofer (ETH Zurich): `https://disco.ethz.ch/members/wroger/AlpenglowPresentation.pdf`
- Alpenglow Whitepaper (Google Drive): `https://drive.google.com/file/d/1Rlr3PdHsBmPahOInP6-Pl0bMzdayltdV/view`
- Reference Implementation: `https://github.com/qkniep/alpenglow/tree/main`
- Stateright Book: `https://www.stateright.rs/title-page.html`
- Learn TLA+: `https://learntla.com/core/index.html`
- Consensus on Solana — Helius: `https://www.helius.dev/blog/consensus-on-solana`
- Accelerate 2025 Talk (YouTube): `https://youtu.be/x1sxtm-dvyE?si=4rsTKAcEuwn8Xcds`


